...
export function CollectionPage({collection, productsToShow, loadMore, productPage, buttonRef }: any) {
  
    return (
      <>
      <div className="max-w-7xl mx-auto justify-center items-center h-full pt-24">
                  <Breadcrumbs title={collection.node.title} crumbmenus={crumbmenus} bg_url={collection.node.image.src} />
                  <div className="flex flex-col w-[80%] mx-auto py-6">
                      <div className="flex flex-col md:flex-row justify-between text-3xl md:text-4xl font-bold py-12 text-gray-800 dark:text-myGray">
                          ...
                          <div className="pt-4 md:pt-0">
                              <p className="text-gray-500 dark:text-gray-400 font-normal mt-2 text-sm">Sort by:
                                  <span className="ml-2">
                          ...
                          {productsToShow?.map((product: any) =>
                              <div className="bg-myGray dark:bg-gray-900 h-full w-full cursor-pointer" key={product.node.id} id={product?.node.handle} onClick={productPage}>
                                  ...
                                  <div className="items-center justify-center  pb-2 mx-auto text-center px-4">
                                      ...
                                      <p className="py-2 text-sm font-normal text-gray-500 dark:text-gray-300 md:px-4">{product?.node.title}</p>
                                      <p className="pb-3 text-xl font-bold text-gray-900 dark:text-gray-300 "><span className="text-[10px] italic text-gray-500 font-light">From </span>${product?.node.priceRange.minVariantPrice.amount} {product?.node.priceRange.maxVariantPrice.currencyCode}</p>
